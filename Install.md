...
This file is for coding agents. If the OpenSKU repository is not already cloned and open, clone `git@github.com:CheungkiCheung/OpenSKU.git` first, then continue from the repository root. If the remote still uses an older fork name, use that actual remote URL and follow the same local steps.

## Goal

Bootstrap an OpenSKU local development workspace on the user's machine with the least risky path available.
...
- The OpenSKU repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch OpenSKU.
...
- If the current directory is not the OpenSKU repository root, clone `git@github.com:CheungkiCheung/OpenSKU.git` if needed, then change into the repository root.
- Confirm the current directory is the OpenSKU repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start OpenSKU
