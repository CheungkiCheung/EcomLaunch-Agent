...
def present_file_tool(
    ...
    if preflight_issues:
        ...
        # Loop-state tracking: record what was tried, why it failed, and how
        # many attempts remain, so the model can revise with full context
        # instead of guessing (TODO.md-style persistent loop state).
        context = getattr(runtime, "context", None)
        if isinstance(context, dict):
            ...
            if isinstance(budget_state, dict):
                ...
                if not isinstance(history, list):
                    history = []
                    budget_state["loop_history"] = history
                ...
                return Command(
                    update={
                        "messages": [
                            ToolMessage(
                                f"{header}{formatted}{loop_context}",
        ...
        # Preflight passed: record a successful delivery in loop state.
        context = getattr(runtime, "context", None)
        if isinstance(context, dict):
            budget_state = context.get(RUN_BUDGET_CONTEXT_KEY)
            if isinstance(budget_state, dict):
