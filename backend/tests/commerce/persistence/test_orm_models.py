...
def test_commerce_tables_use_an_application_owned_metadata_registry():
    ...
    assert ActionArtifactRow.metadata is CommerceBase.metadata
    assert FollowUpRow.metadata is CommerceBase.metadata
...
def test_commerce_tables_compile_for_sqlite_and_postgresql():
    for dialect in (sqlite.dialect(), postgresql.dialect()):
        action_ddl = str(CreateTable(ActionRow.__table__).compile(dialect=dialect))
        artifact_ddl = str(CreateTable(ActionArtifactRow.__table__).compile(dialect=dialect))
        follow_up_ddl = str(CreateTable(FollowUpRow.__table__).compile(dialect=dialect))
        approval_request_ddl = str(CreateTable(ApprovalRequestRow.__table__).compile(dialect=dialect))
        approval_decision_ddl = str(CreateTable(ApprovalDecisionRow.__table__).compile(dialect=dialect))
        case_ddl = str(CreateTable(CaseRow.__table__).compile(dialect=dialect))
        lineage_ddl = str(CreateTable(CaseLineageRow.__table__).compile(dialect=dialect))
        ...
        checkpoint_ddl = str(CreateTable(RunCheckpointRow.__table__).compile(dialect=dialect))
        lease_ddl = str(CreateTable(RunLeaseRow.__table__).compile(dialect=dialect))

        assert "commerce_cases" in case_ddl
        assert "commerce_actions" in action_ddl
        assert "decision_json" in action_ddl
        assert "commerce_action_artifacts" in artifact_ddl
        assert "artifact_json" in artifact_ddl
        assert "commerce_follow_ups" in follow_up_ddl
        assert "follow_up_json" in follow_up_ddl
        assert "commerce_approval_requests" in approval_request_ddl
        assert "commerce_approval_decisions" in approval_decision_ddl
        assert "idempotency_key_sha256" in approval_decision_ddl
        ...
        assert "parent_run_id" in run_ddl
        assert "subject_action_id" in run_ddl
        assert "action_operation" in run_ddl
        assert "requested_paths_json" in run_ddl
