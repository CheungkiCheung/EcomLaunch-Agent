...
def test_preflight_failure_records_loop_state(tmp_path):
    """A failed preflight must write loop_history + a Loop state note into
    the error message so the model knows what was tried and what to fix."""
    outputs_dir = tmp_path / "outputs"
    outputs_dir.mkdir()
    _write_safe_pack(outputs_dir)
    # Corrupt one file so preflight fails deterministically
    (outputs_dir / "competitor-table.csv").write_text(
        "name,evidence_label,source_url\n竞品A,bad_label,https://example.com/a\n",
        encoding="utf-8",
    )
    ...
    result = present_file_tool_module.present_file_tool.func(
        runtime=runtime,
        filepaths=[str(outputs_dir / name) for name in PACK_FILES],
        tool_call_id="tc-loop-1",
    )

    assert "artifacts" not in result.update
    msg = result.update["messages"][0].content
    assert "attempt 1/5" in msg
    assert "[Loop state] This is the first preflight failure" in msg

    budget = runtime.context["__deerflow_agent_run_budget"]
    assert len(budget["loop_history"]) == 1
    assert budget["loop_history"][0]["status"] == "preflight_failed"
    assert budget["loop_history"][0]["issues"]
    assert budget["loop_attempts"] == 1


def test_second_preflight_failure_shows_previous_attempt(tmp_path):
    """The second failure should tell the model what was tried before."""
    outputs_dir = tmp_path / "outputs"
    outputs_dir.mkdir()
    _write_safe_pack(outputs_dir)
    (outputs_dir / "competitor-table.csv").write_text(
    ...
    result2 = present_file_tool_module.present_file_tool.func(
        runtime=runtime,
        filepaths=[str(outputs_dir / name) for name in PACK_FILES],
        tool_call_id="tc-loop-2",
    )

    msg = result2.update["messages"][0].content
    assert "attempt 2/5" in msg
    assert "[Loop state] Previously tried 1 revision(s)" in msg
    assert "bad_label" in msg


def test_loop_exhaustion_stops_revision(tmp_path):
    """After max attempts, the error message must tell the model to stop."""
    outputs_dir = tmp_path / "outputs"
    outputs_dir.mkdir()
    _write_safe_pack(outputs_dir)
    (outputs_dir / "competitor-table.csv").write_text(
    ...
    for i in range(6):
        result = present_file_tool_module.present_file_tool.func(
            runtime=runtime,
            filepaths=[str(outputs_dir / name) for name in PACK_FILES],
            tool_call_id=f"tc-loop-{i}",
        )
    assert "loop exhausted" in result.update["messages"][0].content
    assert runtime.context["__deerflow_agent_run_budget"]["loop_exhausted"] is True


def test_successful_delivery_records_loop_state(tmp_path):
    """A successful delivery should be recorded in loop_history."""
    outputs_dir = tmp_path / "outputs"
    outputs_dir.mkdir()
    _write_safe_pack(outputs_dir)
    ...
    result = present_file_tool_module.present_file_tool.func(
        runtime=runtime,
        filepaths=[str(outputs_dir / name) for name in PACK_FILES],
        tool_call_id="tc-loop-ok",
    )

    assert result.update["artifacts"]
    budget = runtime.context["__deerflow_agent_run_budget"]
    assert budget["loop_delivered"] is True
    assert budget["loop_history"][-1]["status"] == "delivered"
