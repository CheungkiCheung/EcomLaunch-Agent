...
from deerflow.agents.memory.prompt import MEMORY_UPDATE_PROMPT, format_conversation_for_update
...
def test_prepare_update_prompt_uses_agent_scoped_memory_prompt() -> None:
    ...
    with (
        patch(
            ...
            return_value=SimpleNamespace(memory_update_prompt=agent_prompt),
        ),
        patch("deerflow.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True)),
        patch("deerflow.agents.memory.updater.get_memory_data", return_value=_make_memory()),
    ):
        msg = MagicMock()
        msg.type = "human"
        msg.content = "上季度 GMV 320万，转化率 3.2%"
        prepared = MemoryUpdater()._prepare_update_prompt(
            [msg],
    ...
    assert prepared is not None
    _, prompt = prepared
    assert "You are a growth metric memory manager" in prompt
    assert "上季度 GMV 320万，转化率 3.2%" in prompt
    assert MEMORY_UPDATE_PROMPT not in prompt


def test_prepare_update_prompt_falls_back_to_default_without_agent_prompt() -> None:
    """Agents without a scoped prompt keep the default user-profile extraction."""
    with (
        patch(
            "deerflow.config.agents_config.load_agent_config",
            return_value=SimpleNamespace(memory_update_prompt=None),
        ),
        patch("deerflow.agents.memory.updater.get_memory_config", return_value=_memory_config(enabled=True)),
        patch("deerflow.agents.memory.updater.get_memory_data", return_value=_make_memory()),
    ):
    ...
    assert prepared is not None
    _, prompt = prepared
    assert "You are a memory management system" in prompt
