...
Commerce Phase 3 persistence starts under `app/commerce/persistence/`. It reuses the Harness async Engine / Session Factory but owns `CommerceBase`, `commerce_*` tables, an independent Alembic environment and the separate `commerce_alembic_version` table; Harness code must not import these application models. `SqlCaseRepository` provides Workspace-scoped reads and optimistic-concurrency writes. `SqlEvidenceRepository` is append-only and preserves Fact / MetricObservation references; `SqlHypothesisRepository` stores immutable contiguous versions with explicit latest reads. `SqlRunRepository` stores bounded Run projections with optimistic concurrency and Workspace/Case-scoped idempotency hashes; `SqlRunCheckpointRepository` stores append-only, strictly validated `GoalLoopCheckpoint` JSON with per-Run sequence numbers. `SqlRunLeaseRepository` owns one current execution lease per Run, stores only the lease-token SHA-256, increments a fencing token on expired-lease takeover, and rejects stale heartbeat or Checkpoint writes. Production state mutations should use `SqlCommerceUnitOfWork`, which atomically writes Case/Run projections, Evidence/Hypothesis/Checkpoint records and authoritative Domain Events. `SqlDomainEventStore` is append-only, allocates independent Case and Run sequences, retries sequence collisions and requires every Case/Run stream to begin with `case.created`/`run.created`. The feature-flagged `app/commerce/api/` now exposes deterministic multipart Data Intake → Profile → Capability, explicit Workspace semantic confirmation, deterministic Anomaly-to-Case analysis, idempotent Investigation Start and Case/Run/Evidence/Hypothesis/Checkpoint/Event reads without inferring state from chat. New Investigation Runs are created as `queued`; only a successful fenced lease acquisition moves them to `running`. The semantic-candidate endpoint is the only current API path that invokes a model; it runs a fresh identity-verified DeepSeek V4 preflight, records secret-free telemetry, and keeps candidates unconfirmed. Analysis snapshots are stored as immutable derived artifacts linked by MetricObservation IDs. Data files are stored by the existing safe, read-only Intake service and views are recomputed from the immutable Manifest. It currently requires an explicit `X-Commerce-Workspace-Id`; until authenticated Workspace membership is implemented, keep the feature flag disabled for multi-tenant production. SQLite is covered by real repository/migration/rollback/API tests; PostgreSQL is currently covered by SQLAlchemy DDL compilation only, not a live database integration test.
