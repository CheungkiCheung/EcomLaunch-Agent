...
12. **TokenUsageMiddleware** - Records token usage metrics when token tracking is enabled (optional); subagent usage is cached by `tool_call_id` only while token usage is enabled and merged back into the dispatching AIMessage by message position rather than message id. If the optional cache check must load application config and a provider secret referenced by `$ENV_VAR` is unresolved, caching stays disabled so credential-free unit tests can run; unrelated configuration `ValueError`s still propagate.
