...
Commerce Phase 3 persistence starts under `app/commerce/persistence/`. It reuses the Harness async Engine / Session Factory but owns `CommerceBase`, `commerce_*` tables, an independent Alembic environment and the separate `commerce_alembic_version` table; Harness code must not import these application models. `SqlCaseRepository` provides Workspace-scoped reads and optimistic-concurrency writes. `SqlEvidenceRepository` is append-only and preserves Fact / MetricObservation references; `SqlHypothesisRepository` stores immutable contiguous versions with explicit latest reads. Production state mutations should use `SqlCommerceUnitOfWork`, which writes the Case row, Evidence or Hypothesis record, and authoritative Domain Event in one transaction. `SqlDomainEventStore` is append-only, allocates independent Case and Run sequences, retries sequence collisions and supports ordered replay, including `case_version` updates for non-status record events. SQLite is covered by real repository/migration/rollback tests; PostgreSQL is currently covered by SQLAlchemy DDL compilation only, not a live database integration test.
