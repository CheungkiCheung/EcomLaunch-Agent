...
PYTHONPATH=. uv run pytest tests/commerce \
  ...
  --ignore=tests/commerce/agents/test_worker_fulfillment_step_live.py -v
...
截至 2026-07-19，最终加固版预检持续返回 `deepseek-v4-flash`。首个 `FulfillmentPathAgent` 独立验收和 fenced Worker E2E 均使用新鲜 preflight + 新鲜 Agent 请求。最近一次 Worker Agent 请求使用 `3,887` Input Tokens、`474` Output Tokens、单次请求、零重试，约 `4.20s`，并把结构化 Evidence、预算和前后 Checkpoint 写入真实存储。这只证明首条 Fulfillment Worker step，不代表 SellerPeer、ReviewExperience、Lead、Verification、Action/Follow-up 或完整 Gold Case E2E 已经通过。
