"""OpenSKU Sandbox Provisioner Service.
...
K8S_NAMESPACE = os.environ.get("K8S_NAMESPACE", "opensku")
        ...
        if exc.status == 404:
            ns = k8s_client.V1Namespace(
                metadata=k8s_client.V1ObjectMeta(
                    name=K8S_NAMESPACE,
                    labels={
                        "app.kubernetes.io/name": "opensku",
...
app = FastAPI(title="OpenSKU Sandbox Provisioner", lifespan=lifespan)
...
async def list_sandboxes():
    ...
    try:
        services = core_v1.list_namespaced_service(
            K8S_NAMESPACE,
            label_selector="app=opensku-sandbox",
        )
    except ApiException as exc:
        raise HTTPException(
