# OpenSKU Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the OpenSKU backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `opensku` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `opensku` | Kubernetes namespace for sandbox resources |
...
| `USERDATA_PVC_NAME` | empty (use hostPath) | PVC name for user-data volume; existing PVC deployments retain the compatibility `subPath: deer-flow/users/{user_id}/threads/{thread_id}/user-data` layout |
...
Older provisioner versions mounted PVC user-data from `threads/{thread_id}/user-data`. The user-scoped layout retains `deer-flow/users/{user_id}/threads/{thread_id}/user-data` as a compatibility storage prefix; it is a data contract, not the product or Kubernetes namespace.

If an existing deployment already has PVC-backed user-data under the legacy layout, migrate the OpenSKU data directory before relying on the new PVC subPath. Mount the same PVC path that the gateway uses as its OpenSKU base directory, then run the existing user-isolation migration script:
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `opensku` namespace
     - Create/read/delete Services in the `opensku` namespace
     - Read Namespaces (to create `opensku` if missing)
...
docker compose -p opensku-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec opensku-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001","user_id":"user-001"}'

# Check sandbox status
docker exec opensku-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec opensku-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n opensku -l sandbox-id=test-001

# Delete sandbox
docker exec opensku-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec opensku-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec opensku-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec opensku-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/opensku/backend/.deer-flow/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n opensku`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n opensku`
...
3. **Network Isolation**: Sandbox Pods run in the `opensku` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
