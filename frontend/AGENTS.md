# OpenSKU Frontend Agent Architecture

## Overview

OpenSKU uses the [LangGraph SDK](https://github.com/langchain-ai/langgraph) for intelligent, stateful agent interactions. This document outlines the frontend agent architecture, patterns, and working agreements.
...
- The workspace sidebar exposes `OpenSKU Launch Team` and `Growth Analyst` as primary agent entries. Their compatibility route IDs remain `ecom-launch` and `data-inspector`.
...
- The standalone War Room route is `/workspace/war-room`. It visualizes the two primary agents' latest real thread/run state: the OpenSKU Launch Team plus its specialists and the Growth Analyst actor. It must not create a third agent, a second task protocol, or fake activity when no run event exists.
...
This agent architecture is part of OpenSKU.
