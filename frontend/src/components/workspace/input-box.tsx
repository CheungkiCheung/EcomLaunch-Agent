...
export function InputBox({
  ...
  useEffect(() => {
    ...
    // With restricted modes (availableModes set), honor the user's explicit
    // mode choice — the host page switches the model when needed, so do not
    // force a model/mode pair here.
    if (availableModes) {
      if (context.model_name === fallbackModel.name) {
        return;
      }
      onContextChange?.({ ...context, model_name: fallbackModel.name });
      return;
    }
  ...
  }, [context, models, onContextChange, availableModes]);
  ...
  const handleModeSelect = useCallback(
    (mode: InputMode) => {
      ...
      // the default model (deepseek-chat) does not support thinking.
      const resolvedMode = availableModes
        ? mode
        : getResolvedMode(mode, supportThinking);
      onContextChange?.({
        ...context,
        mode: resolvedMode,
        reasoning_effort:
    ...
    [onContextChange, context, supportThinking, availableModes],
  );
            ...
            <PromptInputActionMenu>
              ...
              <PromptInputActionMenuContent className="w-80">
                <DropdownMenuGroup>
                  ...
                  {(!availableModes || availableModes.includes("thinking")) &&
                    supportThinking && (
                      <PromptInputActionMenuItem
                        className={cn(
                          context.mode === "thinking"
