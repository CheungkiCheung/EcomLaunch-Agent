...
function buildMetrics(source: WarRoomSource): WarRoomMetrics {
  const runs = source.ecomRuns ?? [];
  const latest = runs[0];
  const messages = messagesOf(source.ecomThread);
  let webSearches = 0;
  let webFetches = 0;
...
export function buildWarRoomSnapshot(
  ...
  return {
    ...
    stages: buildStages(source, tasks, copy),
    metrics: buildMetrics(source),
    runStatus: source.ecomRunStatus,
    runTitle: source.ecomThread?.values?.title ?? undefined,
  };
}
