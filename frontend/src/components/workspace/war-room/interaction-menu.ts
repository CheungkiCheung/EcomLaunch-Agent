/**
 * RPG-style interaction menu shown when pressing E near a worker.
 * Mouse + keyboard navigable and rendered entirely with Phaser primitives.
 */
...
export class InteractionMenu {
  ...
  private moveSelection(dir: number) {
    ...
    for (let step = 1; step <= n; step++) {
      const idx = (this.selectedIndex + dir * step + n) % n;
  ...
  private updateHighlight() {
    this.highlights.forEach((h, i) => h.setVisible(i === this.selectedIndex));
  }
