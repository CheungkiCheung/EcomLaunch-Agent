...
const ACTIVITY_LABELS: Record<WarRoomActivity, string> = {
  waiting: "等待任务",
  orchestrating: "调度协作",
  searching: "采集信息",
  reading: "读取资料",
  ...
  class WarRoomScene extends Phaser.Scene {
    ...
    private createActor(actor: WarRoomActorSnapshot): ActorRuntime {
      ...
      const statusLabel = this.add
        .text(0, 34, STATUS_LABELS[actor.status], {
          fontFamily:
            'ui-sans-serif, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
          fontSize: "9px",
          color: "#7a6250",
    ...
    private layoutScene = () => {
      ...
      this.background.setPosition(x, y).setDisplaySize(displayWidth, displayHeight);
    ...
    private layoutActor(runtime: ActorRuntime) {
      ...
      const source = runtime.sprite.texture.getSourceImage() as HTMLImageElement;
      const actorWidth = actorHeight * (source.width / source.height);
      runtime.sprite.setDisplaySize(actorWidth, actorHeight);
      runtime.shadow.setSize(actorWidth * 0.56, Math.max(8, actorHeight * 0.09));
    ...
    private moveActor(runtime: ActorRuntime, animate: boolean) {
      ...
      runtime.moveTween = this.tweens.add({
        ...
        onUpdate: () => {
          ...
          runtime.shadow.setScale(0.92 + Math.abs(Math.sin(this.time.now / 85)) * 0.1, 1);
        },
        onComplete: () => {
    ...
    private updateActorAppearance(
      ...
      runtime.statusLabel
        .setText(STATUS_LABELS[actor.status])
      ...
      runtime.bubble
        .setText(
          actor.status === "failed"
            ...
            : actor.status === "done"
              ? "完成啦"
              : ACTIVITY_LABELS[actor.activity],
        )
      ...
      } else if (actor.status === "done" && previousStatus !== "done" && animate) {
        this.tweens.add({
          targets: runtime.sprite,
          y: -10,
          duration: 180,
