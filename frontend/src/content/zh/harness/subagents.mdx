...
除内置子 Agent 外，OpenSKU 还通过 **Agent Connect Protocol (ACP)** 支持委派给外部 Agent。ACP 允许 OpenSKU 调用作为独立进程运行的 Agent（包括用 ACP 适配器包装的第三方 CLI 工具）。
...
<Callout type="tip">
  ACP Agent 作为 OpenSKU 管理的子进程运行，通过 ACP 协议通信。标准 CLI
