---
title: OpenSKU 应用
description: OpenSKU 应用是 OpenSKU 生产体验的参考实现。它将 Harness 运行时、基于 Web 的对话工作区、API Gateway 和反向代理组合成一个可部署的完整系统。
---
...
<Callout type="info" emoji="🚀">
  OpenSKU 应用是构建在 OpenSKU Harness 之上的完整 Super Agent
  应用。它将运行时能力打包成一个可部署的产品，包含 Web 界面、API Gateway
  和运维工具。
</Callout>

OpenSKU 应用是 OpenSKU 生产体验的参考实现。它将 Harness 运行时、基于 Web 的对话工作区、API Gateway 和反向代理组合成一个可部署的完整系统。
...
| Agent 运行时 | LangGraph、LangChain、OpenSKU Harness                  |
| 反向代理     | nginx                                                   |
