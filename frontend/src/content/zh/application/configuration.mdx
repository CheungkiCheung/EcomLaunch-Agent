---
title: 配置
description: 本页面涵盖 OpenSKU 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。
---
...
本页面涵盖 OpenSKU 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。

## config.yaml

`config.yaml` 是 OpenSKU 的主要配置文件。所有 Agent 行为、模型选择、工具加载和运行时功能都由它控制。
...
<Callout type="warning">
  不要为 OpenSKU 工作区文件引入 MCP filesystem server。它会与 OpenSKU
  内置文件工具形成路径语义不同的重复能力，让 LLM 行为不稳定。OpenSKU
...
技能启用状态会反映在 `extensions_config.json` 中。你可以直接编辑它，或通过 OpenSKU 应用界面进行管理。

## 运行时环境变量

这些变量在 OpenSKU 进程中设置（通过 `.env`、Docker 环境或 shell）：

| 变量                    | 默认值           | 描述                                             |
| ----------------------- | ---------------- | ------------------------------------------------ |
| `OPENSKU_CONFIG_PATH` | 自动发现         | `config.yaml` 的绝对路径                         |
| `LOG_LEVEL`             | `info`           | 日志详细程度（`debug`/`info`/`warning`/`error`） |
| `OPENSKU_ROOT`        | 仓库根目录       | 用于 Docker 中的技能和线程挂载                   |
