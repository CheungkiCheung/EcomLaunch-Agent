---
title: 运维与排障
description: 本页面涵盖运行 OpenSKU 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 OpenSKU 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

OpenSKU 应用在 `logs/` 目录中写入每个服务的日志：
...
1. 确保 `backend/.deer-flow/` 对运行 OpenSKU 的进程可写。
2. 在 Docker 部署中，验证卷挂载路径正确（`OPENSKU_ROOT` 设置为绝对路径）。
