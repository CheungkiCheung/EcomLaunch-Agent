---
title: 部署指南
description: 本指南涵盖 OpenSKU 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。
---
...
本指南涵盖 OpenSKU 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。

## 本地开发部署

本地工作流是运行 OpenSKU 最快的方式，所有服务作为原生进程在你的机器上运行。
...
1. 将 `OPENSKU_ROOT` 设置为仓库根目录的绝对路径。
...
OpenSKU 应用使用 [Better Auth](https://www.better-auth.com/) 进行会话管理。在生产环境中：
