---
title: 快速上手
description: 本指南引导你使用 `make dev` 工作流在本地机器上启动 OpenSKU 应用。Gateway、前端和 nginx 会一起启动，通过单个 URL 访问。
---
...
<Callout type="info" emoji="⚡">
  大约 10 分钟即可在本地运行 OpenSKU 应用。你需要一台安装了 Python
  3.12+、Node.js 22+ 的机器，以及至少一个 LLM API Key。
</Callout>

本指南引导你使用 `make dev` 工作流在本地机器上启动 OpenSKU 应用。Gateway、前端和 nginx 会一起启动，通过单个 URL 访问。
