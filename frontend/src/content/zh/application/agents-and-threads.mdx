---
title: Agent 与线程
description: 了解 OpenSKU 中 Agent 与线程的关系，以及如何管理自定义 Agent 和对话线程。
---
...
OpenSKU 允许你创建多个具有不同专业领域的自定义 Agent。每个 Agent 使用 OpenSKU Harness 相同的 Lead Agent 运行时，但具有不同的：
...
OpenSKU 在每次 Agent 轮次后自动保存线程状态（如果配置了检查点器）。这允许：
