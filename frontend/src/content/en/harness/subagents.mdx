...
OpenSKU ships with two built-in subagents:
...
In addition to the built-in subagents, OpenSKU supports delegating to external agents through the **Agent Connect Protocol (ACP)**. ACP allows OpenSKU to invoke agents running as separate processes (including third-party CLI tools wrapped with an ACP adapter).
...
<Callout type="tip">
  ACP agents run as child processes managed by OpenSKU. They communicate over
...
Custom agents created through the OpenSKU App UI can also be invoked as subagents using the `task` tool. When you specify `agent="my-custom-agent"`, the runtime loads that agent's configuration (skills, tool groups, model) and runs it as a subagent for the delegated task.
