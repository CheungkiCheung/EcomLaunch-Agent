...
description: OpenSKU Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
---
...
<Callout type="info" emoji="🔌">
  OpenSKU Harness can be embedded into any Python application. This guide
  covers the integration patterns for using OpenSKU as a library inside your
  own system.
</Callout>

OpenSKU Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
...
# Load configuration (reads config.yaml or OPENSKU_CONFIG_PATH)
load_config()
...
OpenSKU Gateway is itself a FastAPI application. You can mount it as a sub-application or router:
...
# Mount the OpenSKU gateway router
from deerflow.app.gateway.main import app as gateway_app
app.mount("/opensku", gateway_app)
```
...
OpenSKU Harness is built on LangGraph. The Lead Agent is a standard LangGraph graph. You can compose it with your own LangGraph nodes and graphs:
...
```python
import os
os.environ["OPENSKU_CONFIG_PATH"] = "/path/to/my-opensku-config.yaml"
...
```python
from deerflow.config import load_config
load_config(config_path="/path/to/my-opensku-config.yaml")
```

## MCP server integration

OpenSKU can expose its agent as an MCP server, allowing other MCP-compatible systems to call it as a tool. Refer to the OpenSKU repository for MCP server integration examples.
