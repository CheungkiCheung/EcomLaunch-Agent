---
title: Configuration
description: "OpenSKU's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment."
---
...
<Callout type="info" emoji="⚙️">
  All OpenSKU Harness behaviors are driven by <code>config.yaml</code>. One
  file controls which models are available, how the sandbox runs, what tools are
  loaded, and how each subsystem behaves.
</Callout>

OpenSKU's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment.

## Config file location

OpenSKU resolves `config.yaml` using the following priority order:

1. The path passed to `AppConfig.from_file(config_path)` explicitly.
2. The `OPENSKU_CONFIG_PATH` environment variable.
3. `backend/config.yaml` (relative to the backend directory).
4. `config.yaml` in the repository root.

`DEER_FLOW_CONFIG_PATH` remains accepted as a legacy compatibility alias. New deployments and documentation should use `OPENSKU_CONFIG_PATH`.
...
```bash
export OPENSKU_CONFIG_PATH=/path/to/my-config.yaml
```
...
This pattern is how OpenSKU achieves pluggability without hardcoding class references.
