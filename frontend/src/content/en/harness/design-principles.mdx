...
description: Understanding the design principles behind OpenSKU Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
---
...
<Callout type="info" emoji="🏗️">
  OpenSKU is built around one central idea: agent behavior should be composed
  from small, observable, replaceable pieces — not hardcoded into a fixed
  workflow graph.
</Callout>

Understanding the design principles behind OpenSKU Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
...
OpenSKU is a harness because it bundles:
...
OpenSKU is designed for tasks that require more than a single prompt-response exchange. A useful long-horizon agent must:
...
Every architectural decision in OpenSKU is evaluated against this use case. Short, stateless exchanges are easy. Long, multi-step workflows under real-world pressure are the target.

## Middleware chain over inheritance

OpenSKU does not ask you to subclass an agent or override methods to change its behavior. Instead, it uses a **middleware chain** that wraps every LLM turn.
...
Skills also make the system extensible. Adding a new capability to OpenSKU means writing a new skill pack, not modifying the agent core.

## Sandbox is the execution environment

OpenSKU gives agents a **sandbox**: an isolated workspace where they can read files, write outputs, run commands, and produce artifacts.
...
Isolation is important because execution should be reproducible and controllable. The sandbox is the reason OpenSKU can support genuine action rather than only conversation.
...
OpenSKU addresses this through **context engineering** — deliberate control of what the agent sees, remembers, and ignores at each step:
...
All meaningful behaviors in OpenSKU are controlled through `config.yaml`. The system is designed so that operators can change how the agent behaves — which models to use, whether summarization is active, how subagents are limited, what tools are available — without touching code.
