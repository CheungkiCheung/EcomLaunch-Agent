...
description: OpenSKU App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
---
...
OpenSKU App supports multiple named agents and maintains conversation state across sessions through threads and checkpointing.
...
# checkpointer:
#   type: postgres
#   connection_string: postgresql://user:password@localhost:5432/opensku
```
...
In Docker deployments, this path is bind-mounted from the host so that thread data persists across container restarts. Set `OPENSKU_ROOT` to the absolute path of your OpenSKU repository root to ensure the correct host path is used.
