...
description: "This guide covers all supported deployment methods for OpenSKU App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for OpenSKU App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run OpenSKU. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your OpenSKU repo root
export OPENSKU_ROOT=/path/to/opensku
...
# .env
OPENAI_API_KEY=sk-...
OPENSKU_ROOT=/absolute/path/to/opensku
...
1. Set `OPENSKU_ROOT` to the absolute repo root path (or a stable host path).
...
USERDATA_PVC_NAME=opensku-userdata-pvc
SKILLS_PVC_NAME=opensku-skills-pvc
```
...
OpenSKU App uses [Better Auth](https://www.better-auth.com/) for session management. In production:
