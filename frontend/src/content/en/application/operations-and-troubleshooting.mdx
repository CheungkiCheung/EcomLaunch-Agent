...
description: This page covers day-to-day operational tasks and solutions to common problems when running OpenSKU App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running OpenSKU App.
...
When you pull a new version of OpenSKU, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
In Docker deployments, the bind-mounted host path (`$OPENSKU_ROOT/backend/.deer-flow/`) is the source of truth — back up the host path.
