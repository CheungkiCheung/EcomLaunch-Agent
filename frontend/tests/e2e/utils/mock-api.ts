...
export function mockLangGraphAPI(page: Page, options?: MockAPIOptions) {
  ...
  // Auth — return an authenticated user so pages don't redirect to login.
  // The frontend calls /api/v1/auth/me on bootstrap and redirects when the
  // response is 401; without this mock every page lands on the login screen.
