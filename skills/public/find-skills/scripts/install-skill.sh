...
# Find project root by looking for opensku.code-workspace
find_project_root() {
  local dir="$PWD"
  while [[ "$dir" != "/" ]]; do
    if [[ -f "$dir/opensku.code-workspace" ]]; then
      echo "$dir"
      return 0
    fi
    dir="$(dirname "$dir")"
...
if [[ -z "$PROJECT_ROOT" ]]; then
  echo "Error: Could not find project root (opensku.code-workspace not found)"
  exit 1
fi
