#!/usr/bin/env bash
# chat.sh — Send a message to OpenSKU and collect the streaming response.
#
# Usage:
...
#   OPENSKU_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   OPENSKU_URL          — Unified proxy base URL (default: http://localhost:2026)
#   OPENSKU_GATEWAY_URL  — Gateway API base URL (default: $OPENSKU_URL)
#   OPENSKU_LANGGRAPH_URL — LangGraph API base URL (default: $OPENSKU_URL/api/langgraph)
#
# Modes: flash, standard, pro (default), ultra

set -euo pipefail

OPENSKU_URL="${OPENSKU_URL:-${DEERFLOW_URL:-http://localhost:2026}}"
GATEWAY_URL="${OPENSKU_GATEWAY_URL:-${DEERFLOW_GATEWAY_URL:-$OPENSKU_URL}}"
LANGGRAPH_URL="${OPENSKU_LANGGRAPH_URL:-${DEERFLOW_LANGGRAPH_URL:-$OPENSKU_URL/api/langgraph}}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: OpenSKU is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure OpenSKU is running. Start it with: cd <opensku-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from OpenSKU: {data_str}", file=sys.stderr)
            sys.exit(1)
