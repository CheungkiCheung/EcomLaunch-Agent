---
name: claude-to-opensku
description: "Interact with the OpenSKU AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to OpenSKU for research/analysis, start an OpenSKU conversation thread, check OpenSKU status or health, list available models/skills/agents in OpenSKU, manage OpenSKU memory, upload files to OpenSKU threads, or delegate complex research tasks to OpenSKU. Also use when the user mentions opensku, open sku, or wants to run a deep research task that OpenSKU can handle."
---

# OpenSKU Skill

Communicate with a running OpenSKU instance via its HTTP API. OpenSKU is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

OpenSKU exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$OPENSKU_GATEWAY_URL`          | REST endpoints and embedded agent runtime |
| LangGraph-compatible API | 8001 | `$OPENSKU_LANGGRAPH_URL`       | Agent threads, runs, streaming   |
...
|-------------------------|------------------------------------------|------------------------------------|
| `OPENSKU_URL`          | `http://localhost:2026`                  | Unified proxy base URL             |
| `OPENSKU_GATEWAY_URL`  | `${OPENSKU_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `OPENSKU_LANGGRAPH_URL`| `${OPENSKU_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
OPENSKU_URL="${OPENSKU_URL:-http://localhost:2026}"
OPENSKU_GATEWAY_URL="${OPENSKU_GATEWAY_URL:-$OPENSKU_URL}"
OPENSKU_LANGGRAPH_URL="${OPENSKU_LANGGRAPH_URL:-$OPENSKU_URL/api/langgraph}"
```
...
Verify OpenSKU is running:

```bash
curl -s "$OPENSKU_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$OPENSKU_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$OPENSKU_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$OPENSKU_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$OPENSKU_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$OPENSKU_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$OPENSKU_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$OPENSKU_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$OPENSKU_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$OPENSKU_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$OPENSKU_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$OPENSKU_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-opensku/scripts/chat.sh "Your question here"
```
...
- If health check fails, OpenSKU is not running. Inform the user they need to start it.
